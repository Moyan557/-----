...
function BorderGlow({
  ...
  useEffect(() => {
    ...
    animateValue({ duration: 500, onUpdate: (value) => mounted && card.style.setProperty('--edge-proximity', value) });
    animateValue({
      ease: easeInCubic,
      duration: 1500,
      end: 50,
      onUpdate: (value) => {
        if (mounted) card.style.setProperty('--cursor-angle', `${(angleEnd - angleStart) * (value / 100) + angleStart}deg`);
      },
    });
    animateValue({
      ease: easeOutCubic,
    ...
    animateValue({
      ...
      onUpdate: (value) => mounted && card.style.setProperty('--edge-proximity', value),
      onEnd: () => mounted && card.classList.remove('sweep-active'),
    });
